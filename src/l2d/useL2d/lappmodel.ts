...
export class LAppModel extends CubismUserModel {
  private _mouthTimeSeconds = 0;
  private _isMouthMovementEnable = false;
  public toggleLipSync(state: boolean) {
    this._isMouthMovementEnable = state;
  }
  private getFetch() {
    if (this._zipFile) {
      return this.fetchZipFile.bind(this);
    } else {
      return fetch;
    }
  }

  private async fetchZipFile(path: string) {
    if (!this._zipFile) {
      return Promise.reject('no zip file');
    }
    const target = this._zipFile.files[path];

    if (!target) {
      return Promise.reject(`no path in zip file, path: ${path}`);
    }

    const targetArrayBuffer = await target.async('arraybuffer');
    const response = new Response(targetArrayBuffer, {
      status: 200,
    });
  ...
  public loadZipAssets(url: string) {
    this._modelHomeDir = '';
    ...
    fetch(url)
      ...
      .then((zipFile) => {
        ...
        if (!model3JsonName) {
          throw Error('no model3 json');
        }

        this._modelHomeDir = model3JsonName.slice(
          0,
          model3JsonName.lastIndexOf('/') + 1,
        );

        return zipFile.files[model3JsonName].async('arraybuffer');
      })
      .then((arrayBuffer) => {
      ...
      .catch((error) => {
        ...
        CubismLogError(`Failed to load file ${this._modelHomeDir}`);
      });
  }
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    if (this._modelSetting.getModelFileName() != '') {
      ...
      this.getFetch()(`${this._modelHomeDir}${modelFileName}`)
        .then((response) => {
          if (response.ok) {
    ...
    const loadCubismExpression = (): void => {
      if (this._modelSetting.getExpressionCount() > 0) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          this.getFetch()(`${this._modelHomeDir}${expressionFileName}`)
            .then((response) => {
              if (response.ok) {
                return response.arrayBuffer();
              } else if (response.status >= 400) {
    ...
    const loadCubismPhysics = (): void => {
      if (this._modelSetting.getPhysicsFileName() != '') {
        ...
        this.getFetch()(`${this._modelHomeDir}${physicsFileName}`)
          .then((response) => {
            if (response.ok) {
              return response.arrayBuffer();
            } else if (response.status >= 400) {
    ...
    const loadCubismPose = (): void => {
      if (this._modelSetting.getPoseFileName() != '') {
        ...
        this.getFetch()(`${this._modelHomeDir}${poseFileName}`)
          .then((response) => {
            if (response.ok) {
              return response.arrayBuffer();
            } else if (response.status >= 400) {
    ...
    const loadUserData = (): void => {
      if (this._modelSetting.getUserDataFile() != '') {
        ...
        this.getFetch()(`${this._modelHomeDir}${userDataFile}`)
          .then((response) => {
            if (response.ok) {
              return response.arrayBuffer();
            } else if (response.status >= 400) {
  ...
  public startMotion(
    ...
    if (motion == null) {
      this.getFetch()(`${this._modelHomeDir}${motionFileName}`)
        .then((response) => {
          if (response.ok) {
            return response.arrayBuffer();
          } else if (response.status >= 400) {
